...
describe( 'Model', function() {
  ...
  describe( 'diff', function() {
    ...
    it( 'can make creation diffs', function() {
      ...
      let toObj = tbl.get( 1 );
      let model = schema.getModel( toObj._type );
    ...
    it( 'can make update diffs', function() {
      ...
      let model = schema.getModel( fromObj._type );
      let toObj = fromObj.set( 'pages', 400 )
                         .set( 'author', fromObj.author
                                                .add( new ID( {_type: 'author', id: 1} ) )
                                                .delete( new ID( {_type: 'author', id: 2} ) ) );
      let diff = model.diff( fromObj, toObj );
    ...
    it( 'returns undefined if no differences', function() {
      ...
      let tbl = new Table( 'book', {data: data.book, schema, indices: ['id', 'name']} );
      let obj = tbl.get( 2 );
      let model = schema.getModel( obj._type );
