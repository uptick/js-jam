...
// TODO: Remove eventually in favor of code
...
describe( 'Table', function() {
  ...
  describe( 'filter', function() {
    ...
    it( 'works with foreign-keys', function() {
      ...
      let tbl2 = new Table( 'book', {data: data.book, db, indices: ['id', 'title', 'next']} )
      assert.deepEqual( tbl2.filter( {next: db.makeId( 'book', 2 )} ).toJS()[0].id, 1 )
    })

    it( 'works with contains', function() {
      let r = tbl.filter({ name__contains: 'r' }).toJS()
      expect( r.length ).to.equal( 3 )
      expect( r[0].id ).to.equal( 1 )
      expect( r[1].id ).to.equal( 2 )
      expect( r[2].id ).to.equal( 3 )
      r = tbl.filter({ name__contains: 'ran' }).toJS()
